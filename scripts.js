...
function addProductToCart() {
  const id = prompt('Auðkenni vöru:');
  if (id === null) return;

  if (!validateInteger(Number(id), 1)) {
  ...
  if (cartLine) {
    ...
    cart.lines.push({ product, quantity: Number(quantity) });
  }
}
...
function checkout() {
  ...
  if (!name || !address) {
    console.error('Það vantar nafn eða heimilisfang.');
    return;
  }
